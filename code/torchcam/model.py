...
import sys
# print("before swin")
# from swin_unet import Config,SwinUnet
# print("after swin")
...
class InceptionResNetV2(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        self.last_linear1 = nn.Linear(1536, 2)
        # self.classif=nn.Linear(6144, 2)

    def features(self, input):
        # x = self.conv2d_1a(input) #/2
        # x = self.conv2d_2a(x)
        # x = self.conv2d_2b(x)
        # x = self.maxpool_3a(x) #/2
        # x = self.conv2d_3b(x)
        # x = self.conv2d_4a(x)
        # x = self.maxpool_5a(x)  #/2
        # x = self.mixed_5b(x)
        # x = self.repeat(x)     #/2
        # x = self.mixed_6a(x)   #/2
        # x = self.repeat_1(x)
    ...
    def logits(self, features):  # change if errors get
        # x = self.avgpool_1a(features)
        k=features.shape[2]
        x=F.avg_pool2d(features,kernel_size=k)
        x = x.view(x.size(0), -1)
...
def inceptionresnetv2(num_classes=2, pretrained='imagenet'):
